...
- 小于 `768px` 时正文 `p/li` 调整为 `1rem / 1.8`，内联 `span` 继承字号和行高；正文标题 `h1/h2/h3` 分别为 `1.875/1.5/1.25rem`，行高 `1.4`。代码块保留原始换行，宽表格在自身区域横向滚动，单元格最小宽度 `7rem`。
...
- 桌面正文目录在 `top: 6rem` 吸顶，最大高度 `calc(100dvh - 8rem)`；标题固定在目录内部滚动区上方，标题锚点预留 `6rem` 滚动偏移。
- 有目录的页面在 `xl` 起将外框扩至 `1170px`、两侧列扩至 `248px`，正文仍为 `674px`；`lg` 至 `xl` 使用 `160px` 侧栏，小屏继续隐藏目录。目录参考 Innei 的紧凑阅读导航：主章节常显，当前章节的子目录自动展开，其他章节可通过箭头按钮展开。收起分支设置 `inert` 与 `aria-hidden`，包含键盘焦点的分支保持展开。标题完整换行，当前标题使用品牌色、加粗和 `aria-current="location"`；左侧柔光渐隐指示线覆盖当前章节组并平滑移动。底部波浪细线分隔环形阅读进度与回顶按钮。无 JavaScript 时显示完整嵌套目录；动效遵守减少动态效果偏好。
- 首页与归档列表在小于 `768px` 时将日期和标题分行，日期为 `0.875rem`，列表行高为 `1.65`。首页操作按钮按可用宽度自动换行，最小高度为 `44px`。
- 友链页小于 `768px` 时，本站信息卡进入正常文档流，宽度随容器缩放，关闭拖拽，复制按钮常显且点击区域为 `44×44px`。
- 首页轮播在小于 `768px` 时高 `280px`，标题最多三行、摘要最多两行；底部预留 `56px` 给切换按钮，每个圆点的点击区域为 `44×44px`。
...
- 导航搜索、语言和主题切换复用 `IconButton.astro`，桌面按钮统一为 `36×36px`，小屏点击区域为 `44×44px`。图标统一为 `20×20px`、`1.75` 圆头描边；语言使用翻译符号，目标语言由悬停提示和无障碍名称说明，无对应翻译时保留状态圆点。三者使用一致的中性悬停/按下底色与键盘焦点轮廓。
- 主题选择写入 `localStorage.theme`；没有用户选择时跟随 `prefers-color-scheme`。不要改成另一套 `data-theme` 机制而不同时迁移现有 `html.dark` 规则。
- 阅读进度条固定在视口顶部，高度 `2px`，`z-index: 10000`（高于吸顶导航），使用 `--color-primary` 并以 `scaleX` 表示进度。有已初始化目录的桌面文章页隐藏顶部进度条和悬浮回顶按钮，由目录底部环形百分比表示从首个目录标题至正文结尾的进度，并提供回顶入口；移动端和其他页面保留原有进度条与悬浮回顶按钮。
...
- Markdown 容器使用 `prose dark:prose-invert max-w-none`，基础定制规则集中在 `tailwind.config.cjs`，小屏排版覆盖在 `src/styles/global.css`。
...
- 搜索弹层按动态视口高度限制顶部间距和最大高度，长结果在内部滚动；输入框右侧预留 `56px`，关闭按钮为 `44×44px`。重复快捷键保持原始焦点恢复目标，清空查询同时清除忙碌状态。
- 首页轮播在悬停、键盘聚焦、页面隐藏或减少动效开启时暂停自动切换；离开后仅在这些暂停条件均解除时恢复。友链信息卡的复制按钮在键盘聚焦时也保持可见。
